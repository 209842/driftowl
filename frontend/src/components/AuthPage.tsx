      ...
      <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <label htmlFor="terms" style={{ fontSize: 13, color: '#6E6E73' }}>
            I agree to the <Lnk onClick={() => setShowTnC(true)}>Terms & Conditions</Lnk>
          </label>
        </div>
      ...
      {showTnC && (
        <div style={{
          ...
          <div style={{
            ...
            maxHeight: '80vh', overflowY: 'auto', padding: '40px 44px',
            boxShadow: '0 32px 80px rgba(0,0,0,0.25)',
          }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0A0A0F', letterSpacing: -0.5 }}>Terms & Conditions</h2>
              <button onClick={() => setShowTnC(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 22, color: '#8A8A8E', lineHeight: 1 }}>×</button>
            </div>
            {[
              { title: '1. Service', text: 'DriftOwl provides AI-assisted mechanism design analysis for informational and research purposes. Outputs are generated by large language models and do not constitute legal, financial, or professional advice.' },
              { title: '2. Account', text: 'You are responsible for maintaining the security of your account credentials. You must provide accurate information when registering. We reserve the right to suspend accounts that violate these terms.' },
              { title: '3. Acceptable use', text: 'You agree not to use DriftOwl to design mechanisms intended to harm individuals, manipulate democratic processes, facilitate fraud, or violate applicable laws. We may terminate access for violations without notice.' },
              { title: '4. Data & Privacy', text: 'We store your analysis history to provide the Library and history features. We do not sell your data to third parties. Analysis content may be used in anonymized form to improve the service.' },
              { title: '5. Intellectual Property', text: 'The mechanisms and research papers generated are owned by you. DriftOwl retains ownership of the platform, software, and underlying models.' },
              { title: '6. Limitation of Liability', text: 'DriftOwl is provided "as is" without warranties. We are not liable for decisions made based on platform outputs. Use of the service is at your own risk.' },
              { title: '7. Changes', text: 'We may update these terms at any time. Continued use of DriftOwl after changes constitutes acceptance of the revised terms.' },
            ].map(s => (
              <div key={s.title} style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#0A0A0F', marginBottom: 6 }}>{s.title}</div>
                <p style={{ fontSize: 13, color: '#6E6E73', lineHeight: 1.75, margin: 0 }}>{s.text}</p>
              </div>
            ))}
            <div style={{ borderTop: '1px solid rgba(0,0,0,0.07)', paddingTop: 20, marginTop: 8 }}>
              <p style={{ fontSize: 12, color: '#AEAEB2', margin: 0 }}>Last updated: April 2026 · DriftOwl by Enrico Saglimbeni</p>
            </div>
            <button onClick={() => setShowTnC(false)} style={{
              marginTop: 24, width: '100%', padding: '13px', borderRadius: 12, border: 'none',
              background: '#0A0A0F', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer',
            }}>Got it</button>
          </div>
        </div>
      )}
...
function OAuthButtons({ onAuth: _onAuth }: { onAuth: (t: string) => void }) {
    ...
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'flex', gap: 10 }}>
        {[
          ...
          <button key={p.label} type="button"
            onClick={() => { setTooltip('Coming soon'); setTimeout(() => setTooltip(''), 2000) }}
            style={{
              flex: 1, padding: '11px', borderRadius: 12,
              border: '1px solid rgba(0,0,0,0.10)', background: '#fff',
              cursor: 'not-allowed', fontSize: 14, fontWeight: 600, color: '#0A0A0F',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              transition: 'background 0.15s', opacity: 0.65,
            }}
      ...
      {tooltip && (
        <div style={{
          ...
          background: '#0A0A0F', color: '#fff', fontSize: 12, fontWeight: 600,
          padding: '6px 12px', borderRadius: 8, whiteSpace: 'nowrap', pointerEvents: 'none',
        }}>{tooltip}</div>
      )}
    </div>
  )
